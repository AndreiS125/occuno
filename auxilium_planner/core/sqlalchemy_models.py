...
class ConversationThreadModel(Base):
    ...
    # Relationships
    exchanges = relationship("ConversationExchangeModel", back_populates="thread", cascade="all, delete-orphan")

class ConversationExchangeModel(Base):
    """SQLAlchemy model for conversation exchanges"""
    __tablename__ = 'conversation_exchanges'

    id = Column(String(255), primary_key=True)
    thread_id = Column(String(255), ForeignKey('conversation_threads.thread_id', ondelete='CASCADE'), nullable=False)
    user_message = Column(Text, nullable=False)
    planner_summary = Column(Text, default='')
    executor_summary = Column(Text, default='')
    timestamp = Column(DateTime, default=datetime.utcnow)
    final_response = Column(Text, default='')
    execution_metadata = Column(JSONType)
    is_complete = Column(Boolean, default=False)
    
    # Relationships
    thread = relationship("ConversationThreadModel", back_populates="exchanges")
    agent_messages = relationship("ExchangeAgentMessageModel", back_populates="exchange", cascade="all, delete-orphan")
    streaming_events = relationship("ExchangeStreamingEventModel", back_populates="exchange", cascade="all, delete-orphan")
    
    __table_args__ = (
        Index('idx_conversation_exchanges_thread_id', 'thread_id'),
        Index('idx_conversation_exchanges_timestamp', 'timestamp'),
    )

class ExchangeAgentMessageModel(Base):
    """SQLAlchemy model for exchange agent messages"""
    __tablename__ = 'exchange_agent_messages'

    id = Column(String(255), primary_key=True)
    exchange_id = Column(String(255), ForeignKey('conversation_exchanges.id', ondelete='CASCADE'), nullable=False)
    agent = Column(String(50), nullable=False)  # 'planning' or 'executor'
    content = Column(Text, nullable=False)
    message_type = Column(String(50), default='response')  # 'response', 'thinking', 'tool_call', 'tool_result'
    thinking_content = Column(Text, default='')
    tool_calls = Column(JSONType)  # List[Dict]
    timestamp = Column(DateTime, default=datetime.utcnow)
    tool_name = Column(String(255))
    tool_args = Column(JSONType)
    tool_result_parsed = Column(JSONType)
    tool_call_id = Column(String(255))
    
    # Relationships
    exchange = relationship("ConversationExchangeModel", back_populates="agent_messages")
    
    __table_args__ = (
        Index('idx_exchange_agent_messages_exchange_id', 'exchange_id'),
        Index('idx_exchange_agent_messages_agent', 'agent'),
        Index('idx_exchange_agent_messages_timestamp', 'timestamp'),
    )

class ExchangeStreamingEventModel(Base):
    """SQLAlchemy model for exchange streaming events"""
    __tablename__ = 'exchange_streaming_events'
    ...
    # Relationships
    exchange = relationship("ConversationExchangeModel", back_populates="streaming_events")
    
    __table_args__ = (
        Index('idx_exchange_streaming_events_exchange_id', 'exchange_id'),
        Index('idx_exchange_streaming_events_timestamp', 'timestamp'),
    )
